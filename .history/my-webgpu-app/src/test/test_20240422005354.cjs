...
async function test(name, adress){
    ...
    let model = await driver.findElement(By.id(name));
    await model.click();
...
async function runTest() {
    ...
    try {
        await test('home', 'three')
        await test('home', 'babylon')
        await test('home', 'WebGPU')

        await test('car', 'three')
        await test('car', 'babylon')
        await test('car', 'WebGPU')

        await test('snake', 'three')
        await test('snake', 'babylon')
        await test('snake', 'WebGPU')
