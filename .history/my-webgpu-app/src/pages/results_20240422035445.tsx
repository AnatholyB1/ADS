...
export default function Results(props: resultsProps) {
    ...
    const [dataGPUPerTechnology, setDataGPUPerTechnology] = useState<{ labels: string[], datasets: { label: string, data: number[], fill: boolean, backgroundColor: string, borderColor: string }[] } | null>(null);
    ...
    useEffect(() => {
        Papa.parse('/result.csv', {
            ...
            complete: function(results: { data: any[]; }) {
                ...
                const dataGPU = results.data.filter((row: any) => row[' Technology'] === ' Babylon.js with WebGPU').map((row: any) => parseFloat(row[' GPU_Usage_(%)']));
                setDataGPUPerTechnology({
                    labels: labelsPerType,
                    datasets: [
                        {
                            label: 'GPU Usage(%) for babylon.js with WebGPU',
                            data: dataGPU,
                            fill: false,
                            backgroundColor: 'rgb(75, 192, 192)',
                            borderColor: 'rgba(75, 192, 192, 0.2)',
    ...
    return (
        <section className="p-4 grid place-items-center gap-4 grid-flow-row-dense mt-10" {...props}>
                   ...
                   {dataFPSPerTechnology && <Bar  data={dataFPSPerTechnology} />}
                     {dataMemoryPerTechnology && <Bar  data={dataMemoryPerTechnology} />}
                        {dataGPUPerTechnology && <Bar  data={dataGPUPerTechnology} />}
